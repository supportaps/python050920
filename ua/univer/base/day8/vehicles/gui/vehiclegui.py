...
class VehicleGui:
    def __init__(self):
        ...
        self.delete_vehicle_lab = tkinter.Label(self.vehicle_delete_fr,
                                                text='Delete data from database by name: ')

        self.radio_var_del = tkinter.IntVar()
        self.radio_var_del.set(0)
        self.delete_vehicle_but = tkinter.Button(self.vehicle_delete_fr,
                                                 text='Delete', command=self.get_type_vehicle_to_delete)
        self.vehicle_delete_fr.pack()
        self.delete_vehicle_lab.pack(side='left')
        self.delete_vehicle_but.pack(side='left')

        # Frame7 button to generate csv------------------------------------------------------------
        self.vehicle_get_csv_fr = tkinter.Frame(self.main_window)
        self.get_csv_vehicle_lab = tkinter.Label(self.vehicle_get_csv_fr,
                                                 text='Generate csv. from database by name: ')
        self.radio_var_csv = tkinter.IntVar()
        self.radio_var_csv.set(0)
        self.get_csv_vehicle_but = tkinter.Button(self.vehicle_get_csv_fr,
                                                  text='Get csv.', command=self.get_name_to_get_csv)
        self.vehicle_get_csv_fr.pack()
        self.get_csv_vehicle_lab.pack(side='left')
        self.get_csv_vehicle_but.pack(side='left')

        # Frame9 radiobuttons to choose appropriate vehicle-----------------------------
    ...
    def get_type_vehicle_to_delete(self):
        self.radio_var_csv.set(0)
        self.radio_var_del.set(self.radio_var.get())

    def get_name_to_get_csv(self):
        self.radio_var_del.set(0)
        self.radio_var_csv.set(self.radio_var.get())
